...
/* SysTick 配置寄存器 */
#define portNVIC_SYSTICK_CTRL_REG			( * ( ( volatile uint32_t * ) 0xe000e010 ) ) //偏移0x00
#define portNVIC_SYSTICK_LOAD_REG			( * ( ( volatile uint32_t * ) 0xe000e014 ) ) //偏移0x04

//时钟源的配置，是中原在CTRL的Bit2上
...
#define portNVIC_SYSTICK_INT_BIT			( 1UL << 1UL )    //异常请求启动 Bit1位
#define portNVIC_SYSTICK_ENABLE_BIT		( 1UL << 0UL )    //计数器使能	 Bit0位
...
BaseType_t xPortStartScheduler( void )
{
	...
	//注意先初始化systick 如果放到任务之下就跑不进去该代码了
